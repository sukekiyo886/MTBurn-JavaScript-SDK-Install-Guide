...
--- | --- | --- | ---
title_length | タイトルの最大長を指定します。 最大長より長い場合、全体の長さが指定値になるよう、後半が切り取られ、末尾に三点リーダーが付け加えられます。 | `5` | `テストタイトル` -> `テストタ…`
description_length | 説明文の最大長を指定します。 最大長より長い場合、全体の長さが指定値になるよう、後半が切り取られ、末尾に三点リーダーが付け加えられます。 | `10` | `これはテスト説明文です` -> `これはテスト説明文…`
